...
14. **Windows / App Server Sandbox Spawning**: Under constrained Windows sandboxes, test runners or harnesses that spawn nested child processes may be restricted by the platform environment. When this occurs, outcomes are classified as `BLOCKED` or `INCONCLUSIVE` rather than false project test failures. Direct in-process test scripts are recommended where possible.
...
16. **No Host Auto-Execution of Prompt Commands**: CX2 does not automatically execute required verification commands on the host outside the active model turn. The model performs required commands within its standard sandbox and approval contract.
